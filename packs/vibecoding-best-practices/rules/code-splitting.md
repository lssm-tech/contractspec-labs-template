...
- **Audit**: Run `/audit-health` to scan the entire codebase for file size violations and reusability gaps.
- **Refactor**: Run `/refactor <file>` to split oversized files with safety checks.
- **Extract**: Use the `extract-to-bundle` skill when logic needs to move from apps to bundles/libs.
- **Create**: Use the `create-feature` skill to scaffold new features already split and instrumented.
- **Hook**: The `post-edit-checks` hook automatically warns when edited files approach or exceed size limits.
...
- See `observability.md` and `/audit-observability` for logging/analytics requirements in split files
